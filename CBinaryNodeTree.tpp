...
{
	CBinaryNode* m_rootPtr = new CBinaryNode;
...
CBinaryNodeTree<ItemType>::CBinaryNodeTree(const  CBinaryNodeTree<ItemType> &tree)
{
	...
	{
		m_rootPtr = new CBinaryNode<ItemType>;
		m_rootPtr -> SetItem( tree -> GetRootItem() ); //copy root data value
		m_rootPtr -> SetLeftChildPtr( CopyTree( tree -> GetLeftChildPtr() )) //copy left node 
		m_rootPtr -> SetRightChildPtr( CopyTree( tree -> GetRightChildPtr() )) //copy right node
	}
	m_rootPtr = tree.m_rootPtr
	CopyTree(tree.m_rootPtr);
